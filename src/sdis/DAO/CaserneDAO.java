...
public static int RemoveCaserne(Connection connection, Caserne caserne){
            int resultatUpdate = -1;
            try {
                requete=connection.prepareStatement("DELETE FROM CASERNE WHERE CAS_ID = ?");
                requete.setString(1, String.valueOf(caserne.getId()));
...
public static int AddCaserne(Connection connection, Caserne caserne){
            int resultatUpdate = -1;
            try {
                requete=connection.prepareStatement("INSERT INTO CASERNE (CAS_ID, CAS_NOM, CAS_RUE, CAS_CP, CAS_VILLE) VALUES (NULL, ?, ?, ?, ?)");
